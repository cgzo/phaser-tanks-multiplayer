...
var eurecaClientSetup = function() {
    ...
    eurecaClient.exports.spawnEnemy = function(i, x, y) {
        ...
        console.log('SPAWN');
        var tnk = new Tank(i, game, tank, 'enemy');
        tanksList[i] = tnk;
    }
    ...
    eurecaClient.exports.updateState = function(id, state) {
        if (tanksList[id]) {
            ...
            tanksList[id].health = state.health;
            tanksList[id].update();
        }
    }
}
...
Tank = function(index, game, player, tankType) {
    ...
    this.shadow = game.add.sprite(x, y, tankType, 'shadow');
    this.tank = game.add.sprite(x, y, tankType, 'tank1');
    this.turret = game.add.sprite(x, y, tankType, 'turret');
...
Tank.prototype.update = function() {
    var rotation = game.physics.arcade.angleToPointer(turret);
...
function create() {
    ...
    player = new Tank(myId, game, tank, 'tank');
...
function update() {
    ...
    for (var i in tanksList) {
        var curTankListItem = tanksList[i];
        if (!curTankListItem || !curTankListItem.alive) continue;

        if (curTankListItem.health <= 0) {
            curTankListItem.kill();
            curTankListItem.explode();
            continue;
        }
        ...
        for (var j in tanksList) {
            if (!tanksList[j] || !tanksList[j].alive) continue;
            if (j != i) {
                var targetTank = tanksList[j].tank;
                game.physics.arcade.collide(curTank, targetTank);
                game.physics.arcade.overlap(curBullets, targetTank,
                    bulletHitTank, null, this);
            }
            tanksList[i].update();
            tanksList[j].update();
        }
...
function bulletHitTank(tank, bullet) {
    var hitTank = tanksList[tank.name];
    if (hitTank) {
        hitTank.damage();
        hitTank.update();
    } else {
        console.error('Tank ' + tank.name + ' not found!');
    }
    console.log('Tank ' + tank.name + ' has been hit. Health: ' + hitTank.health);
    bullet.kill();
}
